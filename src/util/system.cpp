    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::SMSG:
                ...
                break;
            case OptionsCategory::CPS_WALLET:
                usage += HelpMessageGroup("Capricoin+ wallet Commands:");
                break;
            case OptionsCategory::CPS_STAKING:
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    const int CPS_START_YEAR = 2017;

    std::string sRange = CPS_START_YEAR == COPYRIGHT_YEAR
        ? strprintf(" %i ", COPYRIGHT_YEAR)
        : strprintf(" %i-%i ", CPS_START_YEAR, COPYRIGHT_YEAR);
