...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allow_stakeonly)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Capricoin+ address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("capricoinplus"))
        return false;
...
{
    QString ret = QString("capricoinplus:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CapricoinPlus.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CapricoinPlus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CapricoinPlus (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "capricoinplus.desktop";
    return GetAutostartDir() / strprintf("capricoinplus-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Capricoin+ (%s)\n", chain);
