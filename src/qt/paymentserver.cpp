...
const QString BITCOIN_IPC_PREFIX("capricoinplus:");
#ifdef ENABLE_BIP70
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start capricoinplus: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("capricoinplus://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'capricoinplus://' is not a valid URI. Use 'capricoinplus:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Capricoin+ address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
