...
{
    // Create a message-only window to intercept WM_CLOSE events from capricoinplusd
 ...
 */
static const char* BITCOIN_PID_FILENAME = "capricoinplus.pid";
...
{
#ifdef WIN32
    // Only capricoinplusd will create a hidden window to receive messages
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("capricoinplus-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Capricoin+ GUI in a browser.", false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Capricoinofficial/capricoinplus-core>";
    const std::string URL_WEBSITE = "<https://capricoin.org/>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("capricoinplus-loadblk");
...
{
    fCapricoinPlusMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    if (!fCapricoinPlusMode) {
        WITNESS_SCALE_FACTOR = WITNESS_SCALE_FACTOR_BTC;
        if (gArgs.GetBoolArg("-regtest", false)) {
            ResetParams(CBaseChainParams::REGTEST, fCapricoinPlusMode);
        }
    ...
    // TODO: Check pruning
    if (fPruneMode && fCapricoinPlusMode)
    {
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fCapricoinPlusMode && gArgs.GetBoolArg("-smsg", true)) { // SMSG breaks functional tests with services flag, see version msg
#ifdef ENABLE_WALLET
