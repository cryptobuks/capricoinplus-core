...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (checker.IsCapricoinPlusVersion())
    {
        assert(witness);
        if (scriptSig.size() != 0) {
    ...
    bool fIsP2SH = checker.IsCapricoinPlusVersion() ? scriptPubKey.IsPayToScriptHashAny(checker.IsCoinStake()) : scriptPubKey.IsPayToScriptHash();
