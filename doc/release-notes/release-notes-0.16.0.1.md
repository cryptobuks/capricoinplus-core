Capricoin+ Core version 0.16.0.1 is now available.
...
Capricoin+ Core 0.16 introduces generic support for Hardware Signing Devices to provide key-signing capabilities. 
...
In order to facilitate deterministic generation of stealth addresses between Capricoin+ Core and Signing Devices the method used to generate these addresses has been changed.
...
Note, Legacy Stealth Addresses are not supported on Hardware Signing Devices. If you have CPS/tCPS received on the Legacy Stealth Addresses then we suggest you send those CPS/tCPS to a non-Stealth Address before attempting to use with the Hardware Signing Devices.
...
Capricoin+ Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
capricoinplus data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Capricoin+ Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µCPS" in the GUI now also show the more colloquial term "parts", specified in BIP176.
...
- capricoinplus-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- capricoinplus-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for capricoinplus-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to capricoinplus-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µCPS to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_capricoinplus_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` capricoinplus_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.capricoinplus doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for capricoinplus-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in capricoinplus-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop capricoinplusd (OmeGak)
...
- #10529 `331352f` Improve capricoinplusd systemd service file (Flowdalic)
